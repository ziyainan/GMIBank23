...
import gmibank.pages.US007_CheckEmailPage;
...
public class US007_EmailCheck {

    US007_CheckEmailPage US007_email = new US007_CheckEmailPage();
    ...
    */
    ...
    public void user_login_as_an_admin_by_username_password(String string, String string2) {
        ...
        Driver.waitAndSendText(US007_email.username, string, 4);
        Driver.waitAndSendText(US007_email.password, string2, 4);
        Driver.waitAndClick(US007_email.submit, 4);
    ...
    public void user_click_the_user_info_and_navigate_user_setting_page() {
        Driver.waitAndClick(US007_email.accountmenu, 2);
        Driver.waitAndClick(US007_email.userinfo,2);

    }

    @When("user enter an email {string} which is not contain . extension")
    public void user_enter_an_email_which_is_not_contain_extension(String string) {
        Driver.waitAndSendText(US007_email.pass, string,2);
    }

    @Then("system displays This field is invalid.")
    public void system_displays_This_field_is_invalid() {
        String invalid=Driver.waitAndGetText(US007_email.invalid,2);
//            Assert.assertEquals("This field is invalid",invalid);
    }

    @When("user click the language bar")
    public void user_click_the_language_bar() {
        Driver.waitAndClick(US007_email.language,2);
    }

    @Then("user there is only two language options and these are English or Turkish")
    public void user_there_is_only_two_language_options_and_these_are_English_or_Turkish() {
        Driver.selectAnItemFromDropdown(US007_email.language,"Türkçe");
        Driver.selectAnItemFromDropdown(US007_email.language,"English");
        Select sel=new Select(US007_email.language);
